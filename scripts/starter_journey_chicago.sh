...
assert_only_generated_untracked() {
  local consumer="$1"
  local status
...
# Chicago edge 1: a stranger performs a real anonymous clone of the PR branch.
...
# Chicago edges 4-6: execute real generation, replay it, and independently regenerate.
(
...
assert_only_generated_untracked "$work_root/consumer-a"
...
assert_only_generated_untracked "$work_root/consumer-b"
...
python3 - "$receipt" "$SUBJECT_REPOSITORY" "$SUBJECT_SHA" "$SUBJECT_REF" "$ggen_repository" "$ggen_sha" "$rust_toolchain" "$digest_first" "$digest_replay" "$digest_independent" "$malformed_exit" "$stale_exit" "$os" "$arch" <<'PY'
import json, sys
(path, repo, sha, ref, ggen_repo, ggen_sha, toolchain, first, replay, independent,
 malformed_exit, stale_exit, os_name, arch) = sys.argv[1:]
receipt = {
    "schema_version": 1,
...
printf 'STARTER_JOURNEY_CHICAGO ALIVE subject=%s ggen=%s digest=%s os=%s arch=%s\n' \
  "$SUBJECT_SHA" "$ggen_sha" "$digest_first" "$os" "$arch"
