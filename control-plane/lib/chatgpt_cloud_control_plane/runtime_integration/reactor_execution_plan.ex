defmodule ChatGPTCloud.RuntimeIntegration.ReactorExecutionPlan do
  ...
  @type t :: %__MODULE__{steps: [atom()]}

  @spec admit(t()) :: :ok | {:error, :invalid_reactor_plan}
  def admit(%__MODULE__{steps: steps}) when is_list(steps) and steps != [] do
    if Enum.all?(steps, &(&1 in @allowed)) and length(steps) == length(Enum.uniq(steps)),
      do: :ok,
      else: {:error, :invalid_reactor_plan}
  end
