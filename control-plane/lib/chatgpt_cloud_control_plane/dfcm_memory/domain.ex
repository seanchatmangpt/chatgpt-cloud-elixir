defmodule ChatGPTCloud.DfcmMemory do
  ...
  tools do
    tool :read_dfcm_memory, ChatGPTCloud.DfcmMemory.MemoryRecord, :read do
      description """
      Read-before-manufacture: list current DfCM memory records from Project #2
      (seanchatmangpt/2). Filter by key/kind/cell/standing/tags via the Ash
      query. Call this before proposing or starting any manufacturing work.
      """
    end

    tool :upsert_dfcm_memory, ChatGPTCloud.DfcmMemory.MemoryRecord, :upsert_record do
      description """
      Write-after-manufacture: create-or-update a DfCM memory record by stable
      key (e.g. dfcm/frontier/current, dfcm/measure/latest). Always resolves the
      current record for that key first, so this is an upsert, never a blind
      overwrite. Call this after every manufacturing run, including runs that
      produced no new commits.
      """
    end

    tool :snapshot_dfcm_project, ChatGPTCloud.DfcmMemory.MemoryRecord, :snapshot do
      description """
      Live identity and item/memory-record counts for Project #2
      (seanchatmangpt/2). Read-only. Useful for confirming the junction is
      actually connected before trusting any cached memory read.
      """
    end
  end
