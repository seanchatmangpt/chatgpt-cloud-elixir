...
def main():
    ...
    event_id = str(uuid.uuid5(uuid.NAMESPACE_URL, f"chatgpt-cloud:{agent_id}:{run_id}:{sequence}:{args.activity}"))

    objects = []
    if repo:
        objects.append({"id": f"repo:{repo}", "type": "Repository", "label": repo, "qualifier": "target"})
    if sha:
        objects.append({"id": f"commit:{sha}", "type": "Commit", "label": sha[:12], "qualifier": "subject", "attributes": {"repository": repo}})
